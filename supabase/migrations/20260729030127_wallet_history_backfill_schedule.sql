-- Continuously index bounded historical windows without delaying live balances.
...
begin
  ...
  v_url := v_base_url || case p_worker
    when 'wallet-backfill-worker' then '/spectra-wallet-worker'
    else '/spectra-' || p_worker
  end;
  v_body := case p_worker
    ...
    when 'wallet-backfill-worker' then jsonb_build_object(
      'chains', jsonb_build_array(),
      'limit', 50,
      'mode', 'backfill',
      'runId', 'backfill:' || extensions.gen_random_uuid()::text
    )
...
begin
  for v_job_id in
    select jobid
    from cron.job
    where jobname = 'spectra-wallet-backfill-worker'
  loop
  ...
  perform cron.schedule(
    ...
    '*/5 * * * *',
    $$select spectra_private.invoke_worker_webhook('wallet-backfill-worker')$$
  );
end
$cron$;

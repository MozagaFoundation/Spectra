...
export const VDF_MIN_CHALLENGE_AGE_MS = 15_000
...
function assertProofShape(parsed: ParsedVdfParams, proof: VdfProof): {
  ...
  if (proof.algorithm !== VDF_ALGORITHM || proof.parameterId !== parsed.params.parameterId) {
    throw new Error('Unsupported VDF proof')
  }
  const outputHex = normalizeHex(proof.outputHex)
  const proofHex = normalizeHex(proof.proofHex)
  if (outputHex.length !== parsed.byteLength * 2 || proofHex.length !== parsed.byteLength * 2) {
    throw new Error('Invalid VDF proof length')
  }
  const output = BigInt(`0x${outputHex}`)
  const witness = BigInt(`0x${proofHex}`)
  if (
    output <= 0n ||
    output >= parsed.modulus ||
    witness <= 0n ||
    witness >= parsed.modulus ||
    gcd(output, parsed.modulus) !== 1n ||
    gcd(witness, parsed.modulus) !== 1n
  ) {

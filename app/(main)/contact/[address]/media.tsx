...
import { getConversation, loadCachedMessagesForConversation } from '@/services/chat/chatService'
...
export default function ContactSharedMediaScreen() {
  ...
  const conversation = useMemo(
    () => (address ? getConversation(address, {
      localWalletAddress: local || activeWalletAddress || undefined,
    }) || null : null),
    [activeWalletAddress, address, conversations, local],
  )
  const conversationIds = useMemo(
    () => getDirectConversationIds(address, conversation, conversations),
    [address, conversation, conversations],
  )
  const directMessages = useMemo(
    () => getDirectConversationMessages(messages, conversationIds),
    [messages, conversationIds],
  )
  const sharedContent = useMemo(() => getSharedChatContent(directMessages), [directMessages])

  useEffect(() => {
    if (!address) return
    loadCachedMessagesForConversation(address).catch(() => {})
  }, [address])
  ...
  const prepareAttachment = useCallback(async (item: SharedAttachmentItem): Promise<MediaAttachment> => {
    ...
    try {
      const prepared = await hydrateMessageAttachment(
        ...
        {
          source: 'contactSharedMedia.prepareAttachment',
      ...
      console.warn('Failed to prepare shared attachment:', error)
